...
def lu_factorization_with_complete_pivoting(A):
    n = A.shape[0]
    L = np.eye(n)
    U = A.copy()
    P = np.eye(n)
    Q = np.eye(n)  # Para permutar columnas

    for k in range(n-1):
        # Pivoteo completo: encuentra el elemento con el mayor valor absoluto en el submatriz A[k:n, k:n]
        max_row, max_col = np.unravel_index(np.argmax(abs(U[k:, k:])), U[k:, k:].shape)
        max_row += k
        max_col += k
...
class Structure:
    ...
    def solve_load_pattern(self, load_pattern):
        ...
        A = self.get_stiffness_matrix_modified_by_supports()
        B = f_support

        # Factorización LU con pivoteo completo
        P, L, U, Q = lu_factorization_with_complete_pivoting(A)
        ...
        # Ajuste final para el pivoteo de columnas
        u = Q @ X
